...
def get_stage(index, layers, patch_sizes, embed_dims, hidden_sizes,  state_expansion, block_expansion, conv_dim, headdim, mlp_ratios, block_layer, rnn_layer, mlp_layer,
              norm_layer, act_layer, num_layers, bidirectional, union,
              with_fc, drop=0., drop_path_rate=0., **kwargs):
    if rnn_layer in [MAMBA2D, MAMBA2_2D]:
        ...
        assert len(state_expansion) == len(embed_dims) == len(mlp_ratios) == len(layers), "'state_expansion', 'block_expansion', 'conv_dim' must have the same length as 'embed_dims', 'mlp_ratios', and 'layers'"
    else:
    ...
    for block_idx in range(layers[index]):
        ...
        if rnn_layer == MAMBA2D:
            # Pass the MAMBA-specific parameters
            blocks.append(block_layer(embed_dims[index], state_expansion=state_expansion[index], 
                                      block_expansion=block_expansion[index],
                                      conv_dim=conv_dim[index], mlp_ratio=mlp_ratios[index],
            ...
            blocks.append(block_layer(embed_dims[index], state_expansion=state_expansion[index], 
                                    block_expansion=block_expansion[index], headdim=headdim[index],
                                    conv_dim=conv_dim[index], mlp_ratio=mlp_ratios[index],
                                    rnn_layer=rnn_layer, mlp_layer=mlp_layer, norm_layer=norm_layer,
                                    act_layer=act_layer, num_layers=num_layers,
                                    bidirectional=bidirectional, union=union, with_fc=with_fc,
            ...
            # For other RNN layers, use the regular hidden_size
...
class Sequencer2D(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            get_stage(
                i, layers, patch_sizes, embed_dims, hidden_sizes,  state_expansion, block_expansion, conv_dim, headdim, mlp_ratios, block_layer=block_layer,
...
def sequencer2d_s(pretrained=False, **kwargs):
    model_args = dict(
        ...
        hidden_sizes=[24, 24, 48, 48],
        mlp_ratios=[1.0, 1.0, 1.0, 1.0],
        rnn_layer=MAMBA2D,
        bidirectional=True,
        union="cat",
        with_fc=True,
        state_expansion = [16, 16, 32, 32],  # Example state expansion
